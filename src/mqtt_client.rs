...
impl MQTTClient {
    ...
    fn wait_for_publishes(&self, stream: &mut TcpStream){
        loop {
            ...
            match stream.read_exact(&mut num_buffer) {
                Ok(_) => {
                    ...
                    let topic_name_len: usize = buffer_packet[1] as usize + ((buffer_packet[0] as usize) << 8) as usize;
                    let topic_name = bytes2string(&buffer_packet[2..(2 + &topic_name_len)]);
                    if topic_name.eq(TEMPERATURE_TOPIC){
                        let content = bytes2string(&buffer_packet[(4 + &topic_name_len)..buffer_packet.len()]);
                    }
                }
                Err(_) => {}
            }
    ...
    pub fn run(&self) {
        ...
        self.wait_for_publishes(&mut stream);
    }
}